...
4. ✅ **NATIVE swipeable StackView widget.** DONE (commit 49f22ee) — replaced react-native-android-widget entirely. New local module `modules/echowidget`: StackView deck, `MeshRenderer` draws a REAL-blur mesh (Kotlin Canvas + BlurMaskFilter) + glowing dot/play-icon/waveform bitmaps, crisp native text. Data: RN app pushes cards via `src/lib/echowidget` → `EchoWidgetModule.setCards` → SharedPreferences (`echo_widget`/`cards`); the `RemoteViewsFactory` reads it (no JS context, no network). Play/stop = `PendingIntent` → `NarrationService` (instant); `NarrationService` mirrors `currentId` into `echo_widget`/`playing_id` and broadcasts to `EchoWidgetProvider`. **UNVERIFIED — emulator can't place widgets; verify on the phone:** swipe-deck, mesh look, play/stop icon flip, tap-to-open.
5. ⏳ **FCM push-fresh widget + notifications** — backend pushes when a card turns ready. BLOCKED: needs a Firebase project + `google-services.json` + a service account/server key from the user. A native `FirebaseMessagingService` would post the notification (#3) + call `EchoWidget` refresh even when the app is closed.
